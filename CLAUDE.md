...
账本页流水每一行右侧金额下面写一行小字「余额 X」（千位加逗号），X 为这一笔记完之后的余额：按时间顺序从初始积分累加；手动调整记下时间（`rlzc_ledger.adjust[].ts`，旧数据按「M/D HH:MM」解析），按时间排进楼层流水一起算。
...
14. **设置页折叠规则：** 描述多、选项多的设置卡应可折叠（折叠头包含标题和简要状态，右侧箭头指示展开/收起），默认收起，展开状态持久化到 `extensionSettings.rlzc.cardCollapsed`；副本信息显示位置卡始终展开，不加折叠。副本事件检测卡折叠后在标题右侧保留状态点（如「未测试」）。调试页「<副本> 核对」「手动操作记录」「本次注入」「角色登记」也是折叠卡，默认收起；收起时标题右侧：<副本> 核对没有问题写「无」、有问题写「⚠️」，手动操作记录有记录时写「×N」。
15. **系统页积分：** 回廊卡片里显示积分那一行；副本进行中时状态格下面不显示积分。
...
弹幕：本地弹幕池随机，加上副API定期生成；生成弹幕的调用只读玩家看得到的正文，和事件检测分开调用，绝不共用输入。弹幕默认不给主AI，设置里可开。每一轮放出的弹幕总数10–13条（含AI生成的，不含打赏和系统消息）：没有AI生成的轮次，本地池抽10–13条；有AI生成的轮次，先用AI生成的，不足10条用本地池补到10–13条，超过13条截到13条。
...
- 入场弹窗的「开启直播」上次选择存 `extensionSettings.rlzc.live.optIn`；本局直播记在会话 `chatMetadata.rlzc.live: true`，副本结算或手动结束即下播。手动选择副本的确认框同样带勾选框。
...
- 回廊中的 `<积分变动>` 照常记账，账户校正在回廊收到AI回复后同样清除。
- 核对状态栏积分时不算本楼的直播打赏：打赏在回复写完之后才到账。

### 17.2 AI 弹幕与弹幕注入（实现说明，第三期b-第4段）

- 设置存 `extensionSettings.rlzc.live`：`source`（local / ai）、`freq`（1–10，默认3）、`injectToAI`（默认关）；「直播」卡的折叠状态存 `cardCollapsed.live`。
- AI 弹幕按本场第几轮计：第 N、2N… 轮生成；阶段切换、有人受伤或死亡、本轮注入事件被判定已发生时另加一次；同一轮最多一次。开了事件检测时等检测结果出来再判断。
- 生成结果或失败原因存在该楼 `extra.rlzc.live.ai`，调试页「直播」栏显示。
- 要生成 AI 弹幕的轮次：打赏照常当场记账，这一楼的弹幕和打赏先不放出（本地备用弹幕、打赏暂存在 `live.pending`），AI 返回或失败后按上面的条数规则合成再一起放出。切换聊天、刷新页面时还没等到结果的楼层只用本地池合成。
- `rlzc_live` 深度存 `depths.live`（默认4），内容取当前这一场 feed 的最近5条弹幕与打赏，不含系统消息。

### 17.3 端到端测试后补充（第三期b-第5段）

- 弹幕按节奏放出时，新的一轮开始会把上一轮没放完的直接补齐，不排到新一轮后面。
- 入场弹窗开着时玩家切换了聊天：这次回答不算，也不记拒绝，切回来会再问。
- 状态栏正则里的直播页靠酒馆助手渲染成 iframe，从主页面（逐级向上的 parent）读 `window.RLZC_LIVE`；端到端测试见 `tests/e2e/run-live.mjs`。

## 18. 黑市决定

黑市：面板页签「黑市」，位于「账本」与「设置」之间，分「盘口」「票夹」「赌坊」三个小页签。
盘口：押的是{{user}}自己这一局的故事。入场确认后开盘，入场后第2条AI回复到来时封盘，封盘后不再重开。休整副本（rest: true）不开盘。
盘口分三种：结局盘与评价盘（所有副本都有）；事件盘（写在副本包 markets 字段里）；庄家怪盘（开盘时由副API按公开信息现场出2–3题）。事件盘和庄家怪盘靠副本事件检测判定，事件检测来源为「关闭」时这两种不开。
赔率：赔率 = 1 ÷ 概率 × 0.8（庄家抽水20%）× 0.93–1.07 的随机浮动，保留两位小数，最低1.01，开盘时定下不再变。绝不按剧情真相定赔率。
押注：单个盘口累计押注上限按玩家等级 D 1,000、C 5,000、B 20,000、A 80,000、S 300,000，最低10；不能超过当前可用余额（副本内要扣掉本局直播打赏）。押完余额低于斩杀线时红字提示，不拦截。押自己失败也可以，交给剧情。
赌坊：即开即结的小游戏，共「听钟」「门牌」「抽签」「翻牌」四张桌，每次回到回廊随机摆出两张；默认只在回廊营业，副本包 casino: true 的副本内也营业。所有玩法长期期望都是押注的80%。
流水：下注、兑付、退还、赌坊每局各记一行，账本类型 bet。

### 18.1 黑市接入（实现说明，第四期-第2段）

- `chatMetadata.rlzc_market`：`books`（按会话 id 分开的盘口本：盘口、赔率、赌票、封盘时间、怪盘出题记录）、`casino`（摆桌 `tables`、摆桌时的回廊标识 `key`、每局记录 `plays`）、`hints`（待加的一次性提示）、`seq`（赌票与赌坊共用的先后序号）。
- 盘口 id：结局盘 `ending`、评价盘 `rating`、事件盘用副本包里的 id（M1…）、怪盘 `F1`–`F3`。事件检测输出的 `markets` 以这些 id 为键，judgeNo 为 `id:no`。
- 封盘：入场消息算入场后第1条AI回复，重放出第2条时封盘。
- 开奖由重放得出；结算那一楼不做事件检测，不计入「每一轮都成功检测过」。检测还在进行的那一楼先不开奖。
- 新副本入场时上一局的盘口本定格（已开奖的保留，还没开奖的全退）；入场消息被删或手动删除会话，这一局的赌票全部退。
- 兑付、退还记在开奖的那一楼之后；核对状态栏积分时不算本楼开奖的兑付、退还（回复写完之后才到账）。
- 一次性提示加进一次生成的注入后，收到下一条AI回复时清掉。
- 省钱模式照旧：只在本轮有注入事件、或下一轮有带条件的事件时调用事件检测；没调用的轮次对盘口算「没检测」。

### 18.2 端到端测试后补充（第四期-第4段）

- 第一次下注或开赌坊时把账本初始积分定下来（聊天里还没有状态栏时按默认值记下），免得之后第一条状态栏里已扣过押注的余额被当成初始积分。
- 回到回廊重新摆桌时，新的两张和上一次不完全相同。
- 端到端测试见 `tests/e2e/run-market.mjs`。
